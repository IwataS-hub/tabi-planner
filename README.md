...
日本国内の日帰り〜数日間の旅行を、**地図と旅程**でまとめて計画できるローカルファーストの Web アプリです。行き先を**地名・施設名で検索**するか地図に置き、日ごとに並べ替え、時間・予算・メモを書き込んで、そのまま印刷や PDF にできます。地名検索・住所の自動補完・隣接スポット間の**ルート計算（移動時間と距離の見積もり）**には [Geoapify](https://www.geoapify.com/) を使用します（いずれも任意・明示操作時のみ。API キーが無くても地図クリックでの手動追加や移動時間の手入力は使えます）。
...
- **隣接スポット間のルート計算**（Geoapify）。スポットの間の行に移動手段（徒歩／自動車／自転車／公共交通）を選んで「計算」すると、移動時間・距離・実際のルート形状を取得（公共交通は**参考値**）。明示操作時のみ呼び出し、手入力の移動時間とは区別して「自動／手入力」を表示。公共交通は Geoapify の `approximated_transit` を使う推定経路で、結果がない区間では手入力を利用できます
...
| ルート計算     | **Geoapify Routing API**（任意・明示操作時のみ。公共交通は参考値） |
| 並べ替え       | dnd-kit                                                            |
...
- ルート計算は**明示操作時だけ**呼び出します。結果の time / distance / geometry は検証のうえ、**ルート形状（geometry）は IndexedDB・JSON バックアップへ保存せず**、メモリ（React state とキャッシュ）にのみ保持します。再読み込み後は保存済みの時間・距離だけが残り、地図表示のために API を再取得しません。
...
## ルート計算と移動時間の自動見積もり（Geoapify）

同じ日の隣接するスポット間について、[Geoapify Routing API](https://www.geoapify.com/) で**移動時間・距離・実際のルート形状**を見積もれます。検索・逆ジオコーディングと**同じ API キー**を使います。

- 対応モード: **徒歩 / 自動車 / 自転車 / 公共交通**。公共交通は**参考値**として、Geoapify の `approximated_transit` を利用します。
- 公共交通の推定経路は OpenStreetMap などの利用可能な公共交通データを基にします。実際の時刻表・運賃・乗換案内を保証せず、地域や区間によっては結果が得られない場合があります。その場合も移動時間は手入力できます。
...
- **API キーが未設定でも**アプリは動作します。自動計算は無効になりますが、各スポットの編集で**移動時間を手入力**できます。自動計算した時間と手入力の時間は画面上で「自動／手入力」と区別して表示します。
- 移動手段とリクエストは 1 区間 = 2 地点のみで、advanced details・elevation・avoid・最適化は要求しません。出典は検索 UI と同じく **Powered by Geoapify** を表示します。
...
- ルート計算クライアント（time/distance/geometry 変換・各モード・lang=ja・waypoints は 2 地点・空 route/不正座標の拒否・401/403/429/5xx・timeout/abort/network・キャッシュのヒット/期限切れ/上限）。**実 API は呼びません**
...
- 地名・施設検索／逆ジオコーディング／ルート計算: **Geoapify** — <https://www.geoapify.com/>
