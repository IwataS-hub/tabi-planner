...
完了（Phase 1 / 1.2 / 2.1 / 2.2 / 2.3 / 2.4）:
...
- [x] Trip Toolkit（Phase 2.3）
  - 天気予報（Open-Meteo）— 日次サマリー・開始時刻付きスポットの時間帯天気・手動更新・API キー不要
  - スポット訪問状態（未訪問 / 訪問済み / スキップ）— 旅程リスト・地図マーカー・PlaceEditor で管理
  - 参加者・費用・割り勘（均等・一部均等・カスタム）・精算提案
  - チェックリスト（持ち物 / ToDo）— 追加・編集・完了・削除・未完了フィルター・天気由来の候補追加
  - バックアップに参加者・費用・チェックリスト・訪問状態を含める（JSON v1 互換維持）
- [x] Smart Itinerary（Phase 2.4）
  - **候補スポット箱** — 日程未割当の候補スポットを管理。検索結果から「候補に保存」、任意の日へ昇格、日程内スポットを候補に戻す。ドラッグ並べ替え対応
  - **Day 間移動** — スポットを別の日へ移動（ダイアログで移動先を選択）。移動元・移動先のルート見積もりを自動無効化
  - **予約管理** — 宿泊・交通・レストラン・イベント・アクティビティ・その他を登録。確認番号・URL・電話・メモ・プライベートフラグ。日程・スポットとの紐付け
  - **自動タイムライン** — 開始時刻・滞在時間・移動時間から到着/出発の目安を推定（純粋関数。保存しない）。推定値は「推定」と明記
  - **スマート警告** — 移動時間未入力・過密スケジュール・移動過多・時刻重複・予約衝突・宿泊未登録・雨天屋外・日没後屋外・候補未割当・予算超過（warning / caution / info の 3 段階）
  - **ICS カレンダー書き出し** — Asia/Tokyo タイムゾーンで `.ics` をブラウザダウンロード。確認番号は含めない
  - **旅行複製の強化** — 候補スポット・チェックリスト・参加者・予約をコピー。費用は除外。訪問状態はリセット
  - **旅行テンプレート** — 日帰り・1泊2日・ライブ遠征・グルメ旅行
  - バックアップ v1 互換維持。候補スポット・予約を optional 配列として追加

次の候補:

- [ ] PWA 化（オフライン対応・ホーム画面追加）
- [ ] 複数端末同期、ダークモード、read-only 共有

### プライバシーに関する注意（Phase 2.4 追加）

- **予約の確認番号**は ICS 書き出しには含まれません（タイトル・場所・メモのみ）
- **旅行複製時の予約情報** — 予約は確認番号・isPrivate を含めてそのままコピーされます。不要な場合は複製後に手動で削除してください
- **プライベートフラグ**付き予約はバックアップには含まれますが、将来の共有 export では除外できる設計にしています
- 候補スポットの座標・名称はローカルにのみ保存され、外部 API に送信されません（検索実行時を除く）

## 天気予報（Open-Meteo）

旅程編集画面では、選択中の日の天気予報を表示します。[Open-Meteo](https://open-meteo.com/) の無料 API（API キー不要）を使用し、16 日先まで日次・時間帯別の予報を取得します。

- **API キー不要**。登録なしで利用できます。
- 旅行日程が予報期間（本日から 16 日以内）に含まれる場合のみウィジェットが表示されます。範囲外の旅行や座標のないスポットのみの旅行ではウィジェットを非表示にします。
- **データはメモリキャッシュのみ**（30 分・最大 10 エントリ）。IndexedDB・JSON バックアップには保存しません。
- 「更新」ボタンでキャッシュをクリアして最新を取得できます。
- **プライバシー**: 旅行の座標（スポットの緯度経度）と日程が Open-Meteo に送信されます。個人を特定できる情報は送信しません。

### 天気による持ち物候補（チェックリスト）

チェックリストの「天気から提案」ボタンを押すと、旅行期間の天気予報（雨・猛暑・寒冷・UV・強風など）に基づいて持ち物の候補を提案します。ユーザーが選んだ候補のみ追加されます（重複は自動除外）。
...
- 天気予報: **Open-Meteo**（API キー不要・無料）— <https://open-meteo.com/>
