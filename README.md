...
- **隣接スポット間のルート計算**（Geoapify）。スポットの間の行に移動手段（徒歩／自動車／自転車／公共交通）を選んで「計算」すると、移動時間・距離・実際のルート形状を取得（公共交通は**参考値**）。明示操作時のみ呼び出し、手入力の移動時間とは区別して「自動／手入力」を表示
...
- 対応モード: **徒歩 / 自動車 / 自転車 / 公共交通**。公共交通は**参考値**です（実際の時刻表・運賃は扱いません）。
